 ...
 *   ADD CONSTRAINT `post_lang_fk_1` FOREIGN KEY (`model_id`) REFERENCES `post` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
 * </pre>
 *
 ...
 *             //'localizedPrefix' => 'l_',
 *             'languages' => array('ru', 'uk'), // array of your translated languages
 *             'defaultLanguage' => 'ru', //your main language. Example : 'fr'
 *             //'createScenario' => 'insert',
 *             //'localizedRelation' => 'i18n_post',
 *             //'multiLangRelation' => 'multiLang_post',
 ...
 * </pre>
 *
 * You also can modify the loadModel function of your controller as suggested in a previous post :
 * <pre>
 ...
 * $model = Page::model()->multiLang()->with('articles', 'articles.multiLang_article')->findByPk((int) $id);
...
{
	/**
	 * Example: for a base model class named '\app\models\Post', the translation model class name is '\app\models\PostLang'.
	 *
	 ...
	 */
	public $langForeignKey = 'model_id';
	...
	/**
	 * It must be a simple array: array('fr', 'en')
	 *
	...
	public function attach($owner)
	{
		...
		$simpleTableName = str_replace(array('{{', '}}'), '', array_pop($tableNameChunks));

		if (!isset($this->langClassName)) {
			$this->langClassName = $ownerClassName . 'Lang';
		}
		if (!isset($this->langTableName)) {
			$this->langTableName = $simpleTableName . '_lang';
		}
		if (!isset($this->localizedRelation)) {
			$this->localizedRelation = 'i18n_' . \CHtml::modelName($ownerClassName);
		}
		...
		if ($owner->scenario == $this->createScenario) {
			...
			foreach ($this->languages as $language) {
				foreach ($this->localizedAttributes as $field) {
					...
					$this->setLangAttribute($field . '_' . $language, $owner->$ownerField);
				}
			}
		}
	}
	...
	public function afterSave($event)
	{
		...
		$criteria = new \CdbCriteria();
		$criteria->condition = "{$this->langForeignKey}=:id";
		$criteria->params = array('id' => $ownerPk);
		$criteria->index = $this->langField;
		$languageModels = $model->findAll($criteria);

		foreach ($this->languages as $lang) {
			$defaultLanguage = $lang == $this->defaultLanguage;
			if (!isset($languageModels[$lang])) {
				$languageModel = new $this->langClassName;
				$languageModel->{$this->langField} = $lang;
				$languageModel->{$this->langForeignKey} = $ownerPk;
			} else {
				$languageModel = $languageModels[$lang];
			}
			foreach ($this->localizedAttributes as $field) {
				if ($defaultLanguage) {
					$value = $owner->$field;
				...
				if ($value !== null) {
					$languageField = $this->localizedPrefix . $field;
					$languageModel->$languageField = $value;
				}
			}
