...
\Yii::import('core.components.ActiveRecord');
...
class ModelCode extends \CCodeModel
{
	...
	public $baseClass = '\backstage\components\ActiveRecord';
	public $buildRelations = true;
	public $fixUnsignedInt = true;
	public $preparePropertyColumns = true;
	public $prepareSearchColumns = true;
	public $prepareRules = true;
	...
	public function rules()
	{
		return array_merge(parent::rules(), array(
			...
			array('connectionId, tablePrefix, modelPath, baseClass, buildRelations, fixUnsignedInt, preparePropertyColumns, prepareSearchColumns, prepareRules, multiLang, langModel, seoModel, commentsAsLabels', 'sticky'),
		));
	}
	...
	public function attributeLabels()
	{
		return array_merge(parent::attributeLabels(), array(
			...
			'fixUnsignedInt' => 'Fix Unsigned Int',
			'preparePropertyColumns' => 'Do not describe default fields',
			'prepareSearchColumns' => 'Do not show unnecessary fields in search',
			'prepareRules' => 'Remove from rules unnecessary fields',
			'multiLang' => 'Multi language model',
	...
	public function prepare()
	{
		...
		foreach($tables as $table)
		{
			...
			$params=array(
				...
				'columns'=>$this->fixUnsignedInt ? $this->fixUnsignedInt($table->columns) : $table->columns,
	...
	/**
	 * Fix type of unsigned int fields
	 *
	 * @param $columns
	 *
	 * @return array
	 */
	public function fixUnsignedInt($columns)
	{
		$return = array();
	...
	/**
	 * Remove already described properties
	 *
	 * @param $columns
	 * @return array
	 */
	public function preparePropertyColumns($columns)
	{
		if (!$this->preparePropertyColumns) {
			return $columns;
		}
		$return = array();
		$default = GiiHelper::getDescribedFields();
		foreach ($columns as $key => $column) {
			if (in_array($column->name, $default, true)) {
				continue;
			}
			$item = $column;
			$return[$key] = $item;
	...
	/**
	 * Remove not necessary in search properties
	 *
	 * @param $columns
	 * @return array
	 */
	public function prepareSearchColumns($columns)
	{
		if (!$this->prepareSearchColumns) {
			return $columns;
		}
		$return = array();
		$default = GiiHelper::getNotSearchFields();
		foreach ($columns as $key => $column) {
			if (in_array($column->name, $default, true)) {
				continue;
			}
	 ...
	 * @return null|string
	 */
	public function generateViewRow($page, $column)
	{
		$row = null;
		$doNotShow = GiiHelper::getDoNotShowFields($page);
		if(in_array($column->name, $doNotShow, true))
		{
			return $row;
		}
		$name = $column->name;
		if($page === 'index')
		{
			switch($column)
			{
				...
				case $column->name == 'image_id':
					$row = "array(
						'class' => 'backstage\\components\\ImageColumn',
						'name' => 'image_id',
					),";
					break;
				...
				case $column->name == 'position':
					...
					break;
				case (($column->type == 'integer') || ($column->type == 'string' && $column->size && $column->size < 200)):
					$row = "'{$name}',";
					break;
				default:
					break;
	...
	public function validateBaseClass($attribute,$params)
	{
		...
		if(!is_string($class) || !$this->classExists(ltrim($class, '\\')))
	...
	{
		$labels=array();
		$default = array_keys(GiiHelper::getDefaultLabels());
		foreach($table->columns as $column)
		{
			if(in_array($column->name, $default, true))
			{
	...
	public function generateRules($table)
	{
		...
		$length=array();
		$safe=array();
		$default = GiiHelper::getNotRulesFields();
		foreach($table->columns as $column)
		{
			if ($this->prepareRules && in_array($column->name, $default, true)) {
				continue;
			}
	 ...
	 * @param $table \CDbTableSchema table to inspect
	 ...
	 * @param $tableName string the name of the table to hold the relation
	 * @param $fkName string the foreign key name
	 * @param $multiple boolean whether the relation would contain multiple objects
